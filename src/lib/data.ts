...
export async function getExhibitionBySlug(slug: string): Promise<Exhibition | null> {
  try {
    ...
    if (rawExhibitions.length === 0) return null;
...
export async function getArtistProfile(artistId: string) {
  try {
    ...
    if (artistRes.length === 0) return null;
    ...
    // For each artwork, find all exhibitions it belongs to
...
export async function getCuratorProfile(curatorId: string) {
  try {
    const curatorRes = await db
      .select()
      .from(schema.users)
      .where(eq(schema.users.id, curatorId))
      .limit(1);

    if (curatorRes.length === 0) return null;
    const curator = curatorRes[0] as User;
    ...
    return {
      curator,
      curatedExhibitions: exhibitions,
    };
  } catch (error) {
  ...
  try {
    ...
    for (const curator of curators) {
      result.push({
        ...
        curatedExhibitions: exhibitions.map((e) => ({
          id: e.id,
          title: e.title,
          slug: e.slug,
          status: e.status,
    ...
    console.error('Error fetching curators with stats:', error);
    return [];
  }
}
