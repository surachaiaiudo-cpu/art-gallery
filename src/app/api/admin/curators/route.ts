    ...
    if (searchRes.ok) {
      ...
      if (Array.isArray(files) && files.length > 0) {
        ...
        if (fileId) {
          ...
          console.log('Deleted curator photo from ImageKit:', fileId, filename);
        }
      }
    }
  } catch (err) {
    console.warn('ImageKit delete error for curator:', err);
  }
}
...
// DELETE: Remove curator & clean up ImageKit avatar
